...
public interface DoctorConcernRepository extends JpaRepository<DoctorConcern , Integer> {
    @Query(value = "SELECT distinct d.*,s.schedule_day,s.schedule_time_start,s.fee,s.contact_media,s.scheduleday_parameter FROM concern c,doctors d, doctorconcern dc, schedule s"+
